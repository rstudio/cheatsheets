...
`discrim_flexible(num_terms, prod_degree, prune_method, engine = "earth")` - Fits a discriminant analysis model that uses nonlinear features created using MARS.
...
`discrim_regularized(frac_common_cov, frac_identity, engine = "klaR")` - Estimates a multivariate distribution for the predictors separately for the data in each class. The model's structure can be LDA, QDA, or a combination. Each probability class is computed using Bayes's theorem, given the predictor values.
...
`discrim_linear(penalty)` - Uses Gaussian with a common covariance matrix to perform the estimate.
...
`discrim_quad()` - Uses Gaussian with separate covariance matrices to perform the estimate.
