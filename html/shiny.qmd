...
- `reactive(x)`: Calculate a (reactive) value based on other reactive values. Useful for encapsulating reactive logic needed across multiple outputs. Call the expression with function syntax, e.g. `re()`.
...
- `render*()` functions: Produces results for a corresponding `*Output()` UI container. Re-render occurs when reactive dependencies change. Save the results to `output$<id>`.
