...
-   `shape`: Integer/shape name or a single character ("a").
    ...
    -   `shape` integer/name pairs: 0 = "square open", 1 = "circle open", 2 = "triangle open", 3 = "plus", 4 = "cross", 5 = "diamond open", 6 = "triangle down open", 7 = "square cross", 8 = "asterisk", 9 = "diamond plus", 10 = "circle plus", 11 = "??", 12 = "square plus", 13 = "circle cross", 14 = "square triangle", 15 = "square", 16 = "circle", 17 = "triangle", 18 = "diamond", 19 = "circle small", 20 = "bullet", 21 = "circle filled", 22 = "square filled", 23 = "diamond filled", 24 = "triangle filled", 25 = "triangle down filled" 

<!-- TODO No name for shape #11? Does this formatting work or should it be broken up? -->
...
-   `b + geom_curve(aes(yend = lat + 1, xend = long + 1), curvature = 1)`: Draw a curved line from `(x, y)` to `(xend, yend)`. Aes options: `x`, `xend`, `y`, `yend`, `alpha`, `angle`, `color`, `curvature`, `linetype`, `size`.

-   `a + geom_path(lineend = "butt", linejoin = "round", linemitre = 1)`: Connect observations in the order they appear. Aes options: `x`, `y`, `alpha`, `color`, `group`, `linetype`, `size`.

-   `a + geom_polygon(aes(alpha = 50))`: Connect points into polygons. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `group`, `subgroup`, `linetype`, `size`.

-   `b + geom_rect(aes(xmin = long, ymin = lat, xmax = long + 1, ymax = lat + 1))`: Draw a rectangle by connecting four corners (`xmin`, `xmax`, `ymin`, `ymax`). Aes options: `xmax`, `xmin`, `ymax`, `ymin`, `alpha`, `color`, `fill`, `linetype`, `size`.

-   `a + geom_ribbon(aes(ymin = unemploy - 900, ymax = unemploy + 900)`: For each `x`, plot an interval from `ymin` to `ymax`. Aes options: `x`, `ymax`, `ymin`, `alpha`, `color`, `fill`, `group`, `linetype`, `size`.
...
-   `c + geom_area(stat = "bin")`: Draw an area plot. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `linetype`, `size`.

-   `c + geom_density(kernel = "gaussian")`: Compute and draw kernel density estimates. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `group`, `linetype`, `size`, `weight`.

-   `c + geom_dotplot()`: Draw a dot plot. Aes options: `x`, `y`, `alpha`, `color`, `fill`.

-   `c + geom_freqpoly()`: Draw a frequency polygon. Aes options: `x`, `y`, `alpha`, `color`, `group`, `linetype`, `size`.

-   `c + geom_histogram(binwidth = 5)`: Draw a histogram. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `linetype`, `size`, `weight`.

-   `c2 + geom_qq(aes(sample = hwy))`: Draw a quantile-quantile plot. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `linetype`, `size`, `weight`.
...
-   `d + geom_bar()`: Draw a bar chart. Aes options: `x`, `alpha`, `color`, `fill`, `linetype`, `size`, `weight`.
...
-   `e + geom_label(aes(label = cty), nudge_x = 1, nudge_y = 1)`: Add text with a rectangle background. Aes options: - `x`, `y`, `label`, `alpha`, `angle`, `color`, `family`, `fontface`, `hjust`, `lineheight`, `size`, `vjust`.

-   `e + geom_point()`: Draw a scatter plot. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `shape`, `size`, `stroke`.

-   `e + geom_quantile()`: Fit and draw quantile regression for the plot data. Aes options: `x`, `y`, `alpha`, `color`, `group`, `linetype`, `size`, `weight`.

-   `e + geom_rug(sides = "bl")`: Draw a rug plot. Aes options: `x`, `y`, `alpha`, `color`, `linetype`, `size`.

-   `e + geom_smooth(method = lm)`: Plot smoothed conditional means. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `group`, `linetype`, `size`, `weight`.

-   `e + geom_text(aes(label = cty), nudge_x = 1, nudge_y = 1)`: Add text to a plot. Aes options: `x`, `y`, `label`, `alpha`, `angle`, `color`,
...
-   `f + geom_col()`: Draw a bar plot. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `group`, `linetype`, `size`.

-   `f + geom_boxplot()`: Draw a box plot. Aes options: `x`, `y`, `lower`, `middle`, `upper`, `ymax`, `ymin`, `alpha`, `color`, `fill`, `group`, `linetype`, `shape`, `size`, `weight`.

-   `f + geom_dotplot(binaxis ="y", stackdir = "center")`: Draw a dot plot. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `group`.

-   `f + geom_violin(scale = "area")`: Draw a violin plot. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `group`, `linetype`, `size`, `weight`.
...
-   `g + geom_count()`: Plot a count of points in an area to address over plotting. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `shape`, `size`, `stroke`.

-   `e + geom_jitter(height = 2, width = 2)`: Jitter points in a plot. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `shape`, `size`.
...
-   `h + geom_bin2d(binwidth = c(0.25, 500))`: Draw a heatmap of 2D rectangular bin counts. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `linetype`, `size`, `weight`.

-   `h + geom_density_2d()`: Plot contours from 2D kernel density estimation. Aes options: `x`, `y`, `alpha`, `color`, `group`, `linetype`, `size`.

-   `h + geom_hex()`: Draw a heatmap of 2D hexagonal bin counts. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `size`.
...
-   `i + geom_area()`: Draw an area plot. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `linetype`, `size`.

-   `i + geom_line()`: Connect data points, ordered by the x axis variable. Aes options: `x`, `y`, `alpha`, `color`, `group`, `linetype`, `size`.

-   `i + geom_step(direction = "hv"`: Draw a stairstep plot. Aes options: `x`, `y`, `alpha`, `color`, `group`, `linetype`, `size`.
...
-   `j + geom_crossbar(fatten = 2)`: Draw a crossbar. Aes options: `x`, `y`, `ymax`, `ymin`, `alpha`, `color`, `fill`, `group`, `linetype`, `size`.

-   `j + geom_errorbar()`: Draw an errorbar. Also `geom_errorbarh()`. Aes options: `x`, `ymax`, `ymin`, `alpha`, `color`, `group`, `linetype`, `size`, `width`.

-   `j + geom_linerange()`: Draw a line range. Aes options: `x`, `ymin`, `ymax`, `alpha`, `color`, `group`, `linetype`, `size`.

-   `j + geom_pointrange()`: Draw a point range. Aes options: `x`, `y`, `ymin`, `ymax`, `alpha`, `color`, `fill`, `group`, `linetype`, `shape`, `size`.

<!-- TODO Are these descriptive enough? -->
...
<!-- The code chunk here uses functions from the maps package -->

-   `k + geom_map(aes(map_id = state), map = map) + expand_limits(x = map$long, y = map$lat)`: Draw polygons as a map. Aes options: `map_id`, `alpha`, `color`, `fill`, `linetype`, `size`.
...
-   `l + geom_contour(aes(z = z))`: Draw 2D contour plot. Aes options: `x`, `y`, `z`, `alpha`, `color`, `group`, `linetype`, `size`, `weight`.

-   `l + geom_contour_filled(aes(fill = z))`: Draw 2D contour plot with the space between lines filled. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `group`, `linetype`, `size`, `subgroup`.

-   `l + geom_raster(aes(fill = z), hjust = 0.5, vjust = 0.5, interpolate = FALSE)`: Draw a raster plot. Aes options: `x`, `y`, `alpha`, `fill`.

-   `l + geom_tile(aes(fill = z))`: Draw a tile plot. Aes options: `x`, `y`, `alpha`, `color`, `fill`, `linetype`, `size`, `width`.
...
<!-- TODO What here should be code text? -->
