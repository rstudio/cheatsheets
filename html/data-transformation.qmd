...
-   `filter_out(.data, ..., .preserve = FALSE)`: Drop rows that meet logical criteria (the complement to `filter()`).
    Missing values in the dropped condition are kept by default, so you don't need to guard against `NA` explicitly.

```{r}
mtcars |> filter_out(mpg > 20)
```

-   Use `when_any()` and `when_all()` inside `filter()` to combine multiple conditions.

```{r}
mtcars |> filter(when_any(mpg > 30, hp > 300))
```
...
-   `dplyr::case_when()`: multi-case `if_else()`.
    Use `.unmatched = "error"` to error on any unmatched cases instead of silently returning `NA`.
...
-   `dplyr::replace_when()`: replace values based on a logical condition, like a targeted `case_when()`.

```{r}
starwars |> mutate(name = replace_when(name, height > 200 ~ "Tall"))
```

-   `dplyr::coalesce()`: first non-NA values by element across a set of vectors

-   `dplyr::recode_values()`: recode values into a new column, using either a formula interface (like `case_when()`) or `from`/`to` lookup vectors.

```{r}
starwars |>
  mutate(size = recode_values(species, "Droid" ~ "robot", default = "other"))
```

-   `dplyr::replace_values()`: replace specific values within an existing column, similar to `coalesce()`, `na_if()`, or `tidyr::replace_na()`.

```{r}
starwars |> mutate(species = replace_values(species, NA ~ "Unknown"))
```
