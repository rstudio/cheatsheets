...
-   `f + geom_boxplot()`: Draw a box plot.
    ...
    Style individual parts with `median.*`, `box.*`, `whisker.*`, and `staple.*` arguments.
...
-   `f + geom_violin(scale = "area")`: Draw a violin plot.
    `aes()` arguments: `x`, `y`, `alpha`, `color`, `fill`, `group`, `linetype`, `linewidth`, `weight`.
    Use `quantiles` to compute quantile lines and `quantile.colour`/`quantile.linetype`/`quantile.linewidth` to style them (blank by default).
...
-   Use `reverse = "none"` (default), `"x"`, `"y"`, or `"xy"` with any coord to flip an axis direction.
...
-   Customize complete themes with `ink` (foreground), `paper` (background), and `accent` color arguments that propagate to layer defaults (e.g., `theme_gray(ink =, paper =, accent =)`).

```{r}
u + theme_gray(paper = "cornsilk", ink = "navy", accent = "tomato")
```

-   `u + theme()`: Customize aspects of the theme such as axis, legend, panel, and facet properties. Use `theme(geom = element_geom(...))` to set the same defaults without switching themes, and `from_theme(accent)` inside `aes()` to map a layer to them.
...
-   `u + theme_sub_panel()`: Shortcut for clusters of `theme()` settings that share a prefix. Also `theme_sub_axis()`, `theme_sub_legend()`, `theme_sub_plot()`, `theme_sub_strip()`, and directional variants like `theme_sub_axis_x()`.
...
Or label variables from a named vector (e.g., from a data dictionary), keyed by variable name rather than aesthetic.

```
t + labs(dictionary = c(cty = "City mileage (mpg)", hwy = "Highway mileage (mpg)"))
```
