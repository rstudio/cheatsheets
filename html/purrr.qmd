...
TODO:
* Replace all `~` with `\(x)`
* Add some explanation for map functions 
* Add row for `_vec`
* Need to introduce `list_c()` and friends (https://www.tidyverse.org/blog/2022/12/purrr-1-0-0/#flattening-and-simplification)
* Filter -> predicate functions
* Add `keep_at` and `discard_at`
* `flatten` -> `list_flatten()`, `transpose()` -> `list_transpose()`
--->
...
|-----------|-------------------------------------|------------------------------------------------------|------------------------------------------------------------|
| Logical   | `map_lgl(x, is.integer)`            | `` map2_lgl(l2, l1, `%in%`) ``                       | `` pmap_lgl(list(l2, l1), `%in%`) ``                       |
| Integer   | `map_int(x, length)`                | `` map2_int(y, z, `+`) ``                            | `` pmap_int(list(y, z), `+`) ``                            |
| Double    | `map_dbl(x, mean)`                  | `map2_dbl(y, z, ~ .x / .y)`                          | `pmap_dbl(list(y, z), ~ .x / .y)`                          |
| Character | `map_chr(l1, paste, collapse = "")` | `map2_chr(l1, l2, paste, collapse = ",", sep = ":")` | `pmap_chr(list(l1, l2), paste, collapse = ",", sep = ":")` |
| No output | `walk(x, print)`                    | `walk2(objs, paths, save)`                           | `pwalk(list(objs, paths), save)`                           |
